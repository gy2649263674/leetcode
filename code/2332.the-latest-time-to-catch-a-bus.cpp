...
{
public:
    int latestTimeCatchTheBus(vector<int> &buses, vector<int> &psg, int capacity)
    {
        sort(buses.begin(), buses.end());
        sort(psg.begin(), psg.end());
        deque<int> dpsg(1, 0);
        dpsg.insert(dpsg.end(), psg.begin(), psg.end());
        int ans = -1, b = 0, l = 1;
        bool ex = false;
        for (int b = 0; b < buses.size(); b++)
        {
            int cnt = 0;
            while (l<dpsg.size()&&dpsg[l] <= buses[b] && cnt < capacity)
            {
                if (dpsg[l] - dpsg[l - 1] > 1)
                {
                    ans = max(ans, dpsg[l] - 1);
                }
                l++;
                cnt++;
            }
            // eq to ..-2 then has a arrive at -1
            if(dpsg[l-1]!= buses[b]&&cnt<capacity)
            {
                ans = max(ans,buses[b]);
            }
        }
...
{
    Solution s;
    cout<<s.latestTimeCatchTheBus({2,3,4},{2},3);
    return 0;
}
/*
// @lcpr case=start
